...
**변경 감지 정합성 대규모 개선 (파서)**: HEAD 대비 4개 CSV 합계 **1,592 → 1,204행 (388건 감소)**. 감소는 대부분 "원래 없어야 할 행" — 가짜 변경/반복·가짜 폐지/재-신설. 사라진 정보는 비고 컬럼으로 추적 보존.

**1. 비교 정규화 (`_norm_compare`)** — 시각적으로 같은 기호 변이를 통일해 가짜 "변경"을 제거. NFKC가 기호를 분해해 흩어지므로 **NFKC 전에** 통일.
- 점·가운뎃점 11종(`·‧∙•・⋅･ㆍᆞ` + 그리스 U+0387, one-dot-leader U+2024 등) → `·`
- 대시·하이픈 9종(`-–—―−－` 등) → `-`
- 물결표 5종(`~∼〜～⁓`) → `~`
- 괄호 `[]{}` → `()`
- 쉼표는 의미 보존 위해 **정규화 안 함** (대신 표현 정비 비고 참고)

**2. 파서 버그 수정 — 줄바꿈 누락**: `cell_text`가 `t.text`만 읽어 `<hp:lineBreak/>` 뒤 텍스트를 버림(예 `발전<lineBreak/>시스템`→`발전`). `itertext()`로 교체해 42셀 복구(`나. 발전시스템`, 잘렸던 설명 등).

**3. 반복 삭제행 가드**: 법령 별표가 폐지 항목을 개정마다 "삭제" 자리표시로 남겨 매 버전 삭제행이 중복 생성되던 문제. 직전이 이미 삭제면 생략.

**4. 적용시기 데이터셋별 override (`DATASET_APPLY_OVERRIDE`)**: 국가전략기술 tech(별표7의2)·시설(별표6의2) 최초 신설을 제도 시행일 **2021-07-01**로 보정.

**5. 분야 번호 canonical 재정비 (`STRATEGIC_SECTOR_NO`/`NEWGROWTH_SECTOR_NO`)**: 첫등장순 누적 대신 정규화 분야명→확정번호 맵. 개명 전후를 같은 번호로 병합해 **기술 연혁 보존**(예: SW 3↔12, 에너지 8↔14 병합). 비고 각주가 분야로 오파싱돼 유령 번호(16/18)를 먹던 문제 제거 → 신성장 1~14(방위산업=14), 전략 1~8(미래형운송=6, 인공지능=8). 이름은 **변경 없는 최신 버전 포함 전 버전 items 기준**으로 통일(시설 에너지가 신ㆍ환경에 굳던 버그 수정 → 에너지ㆍ환경).

**6. 한글 열거기호 정렬 (`_kor_marker_order`)**: 항목/소분류 `가.나.다…하.거.너.더…`(열거 순). 자음14×모음라운드(ㅏㅓㅗㅜㅡㅣ). 파서 CSV 정렬 + 프런트엔드(`techOrdering.js`)에 적용. (이전엔 `가.거.나.너` 자음 묶음으로 정렬됨)

**7. 프런트엔드 정렬 (`compareStatuteOrder`)**: 소분류를 전체 이름이 아닌 **가나다 prefix**로 정렬 — 개명으로 옛 이름 단 항목이 엉뚱한 위치로 밀리던 문제(대기압 플라즈마 7→2 등) 수정.

**8. 비고(`note`) 컬럼 — 마지막 열, 모든 비고는 적용시기로 시작**:
- **개명**: `2026-01-01 '에너지 신ㆍ환경'에서 '에너지ㆍ환경'으로 분야명 변경` (변경 없는 버전 포함 전 버전 추적)
- **이동** (통일 포맷 `{출발}에서 {목적지}(으)로 이동`, 출발·도착 **양쪽 행 동일 표기**):
  - 데이터셋 간 상향: `신성장·원천기술 13. 탄소중립 - 나. 수소에서 국가전략기술 5. 수소로 이동`
  - 분야 간 / 분야 내 소분류 간 / 같은 소분류 번호 이동 모두 동일 형식
  - 번호 없는 항목: `같은 소분류 내 위치 이동`
- **표현 정비**: `표현 정비: '위하여'→'위해'` (어미 축약만 다른 변경)
- 가운뎃점 좌우 공백 정리(`_tidy_name`: 에너지 ㆍ환경 → 에너지ㆍ환경)
- 원래 위치는 정규화 전 **원본 이력명**으로 정확히 기록

**9. `has_facility` 매칭을 분야명 → canonical 분야번호 기준**으로 변경(이름 변형에 안 깨짐).

**빌드 순서**: `build_tech_history.py`(상향 비고 포함) → `build_facility_history.py`(has_facility 추가, note 맨끝 유지) 순서 필수.

**보류**: 슬롯 재활용(9족 금속 등) 기술 단위 정밀 매칭은 `input/subsector_merge_todo.md`에 목록화. 비고가 진짜 개명/슬롯재활용 판별 자료.
